...
export default {
  ...
  methods: {
    ...
    async blurFun(item) {
      this.focusFun(item)
      let info = await this.axios.post('/log/field-value', { pro_code: 2000, order_no: this.$route.query.order_no || '', field_name: item.code, field_value: JSON.stringify(item[item.code]) });
    },
